...
function sod_apply_manual_override_to_analyzed(array $analyzed, array $row): array {
    ...
    $wd['evaluation_label'] = 'ÙŠØ¯ÙˆÙŠ Ù…Ù‚ÙÙ„';
    ...
    $fd['evaluation_meta'] = ['mode'=>'manual_override','label'=>'ÙŠØ¯ÙˆÙŠ Ù…Ù‚ÙÙ„'];
...
function sod_mark_evaluation_state(array $row, array $update, string $mode = 'auto'): array {
    ...
    $label = 'Ø¢Ù„ÙŠ';
    if ($mode === 'manual_override') $label = 'ÙŠØ¯ÙˆÙŠ Ù…Ù‚ÙÙ„';
    elseif ($mode === 'manual_saved') $label = 'Ø­ÙÙØ¸ ÙŠØ¯ÙˆÙŠÙ‹Ø§';
...
function sod_attach_manual_override_state(array $row, array $update, array $fields, string $editor = ''): array {
    ...
    $fd['evaluation_meta'] = ['mode'=>'manual_override','label'=>'ÙŠØ¯ÙˆÙŠ Ù…Ù‚ÙÙ„','at'=>time()];
    $update['war_data'] = wp_json_encode($wd, JSON_UNESCAPED_UNICODE);
    $update['field_data'] = wp_json_encode($fd, JSON_UNESCAPED_UNICODE);
    return $update;
}
...
function sod_ajax_newslog_search(): void {
    ...
    $unknown_actor_values = ['','ØºÙŠØ± Ù…Ø­Ø¯Ø¯','Ø¹Ø§Ù…/Ù…Ø¬Ù‡ÙˆÙ„','ÙØ§Ø¹Ù„ ØºÙŠØ± Ù…Ø­Ø³ÙˆÙ…','ÙØ§Ø¹Ù„ Ù‚ÙŠØ¯ Ø§Ù„ØªÙ‚ÙŠÙŠÙ…','Ø¬Ù‡Ø© ØºÙŠØ± Ù…Ø¹Ù„Ù†Ø©','ÙØ§Ø¹Ù„ Ø³ÙŠØ§Ù‚ÙŠ','ÙØ§Ø¹Ù„ Ø³ÙŠØ§Ù‚ÙŠ ØºÙŠØ± Ù…Ø¨Ø§Ø´Ø±'];
    ...
    if ($items) {
        ...
        foreach ($items as &$it) {
            ...
            $it['evaluation_label'] = (string)($wd['evaluation_label'] ?? (!empty($manual_state['enabled']) ? 'ÙŠØ¯ÙˆÙŠ Ù…Ù‚ÙÙ„' : 'Ø¢Ù„ÙŠ'));
            ...
            if (
                ...
                in_array($it['actor_v2'], ['', 'ØºÙŠØ± Ù…Ø­Ø¯Ø¯', 'Ø¹Ø§Ù…/Ù…Ø¬Ù‡ÙˆÙ„', 'ÙØ§Ø¹Ù„ ØºÙŠØ± Ù…Ø­Ø³ÙˆÙ…', 'ÙØ§Ø¹Ù„ Ø³ÙŠØ§Ù‚ÙŠ', 'ÙØ§Ø¹Ù„ Ø³ÙŠØ§Ù‚ÙŠ ØºÙŠØ± Ù…Ø¨Ø§Ø´Ø±', 'Ø¬ÙŠØ´ Ø§Ù„Ø¹Ø¯Ùˆ Ø§Ù„Ø¥Ø³Ø±Ø§Ø¦ÙŠÙ„ÙŠ'], true)
            ) {
...
function sod_ajax_newslog_reclassify(): void {
    ...
    $candidate_where = "actor_v2 IN ('" . implode("','", array_map('esc_sql', $unknown_actor_values)) . "') OR (actor_v2='Ø¬ÙŠØ´ Ø§Ù„Ø¹Ø¯Ùˆ Ø§Ù„Ø¥Ø³Ø±Ø§Ø¦ÙŠÙ„ÙŠ' AND title REGEXP 'ØªØ±Ø§Ù…Ø¨|ØªØ±Ù…Ø¨|Ù†ØªÙ†ÙŠØ§Ù‡Ùˆ|Ø§Ù„Ø¨ÙŠØª Ø§Ù„Ø£Ø¨ÙŠØ¶|Ø§Ù„ØªÙ„ÙØ²ÙŠÙˆÙ† Ø§Ù„Ø¥ÙŠØ±Ø§Ù†ÙŠ|Ø¥ÙŠØ±Ù†Ø§|Ø§Ù„Ø±Ø¦Ø§Ø³Ø©|ÙˆÙØ¯|Ù…ÙØ§ÙˆØ¶Ø§Øª|Ù…Ø­Ø§Ø¯Ø«Ø§Øª|Ø§Ø¬ØªÙ…Ø§Ø¹|Ù„Ù‚Ø§Ø¡|Ø±ÙˆÙŠØªØ±Ø²|Ù…ØµØ¯Ø±|ØªØºØ·ÙŠØ©|Ù‚Ù†Ø§Ø©|Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©|Ø§Ù„Ù…ÙŠØ§Ø¯ÙŠÙ†|Ø§Ù„Ø¬Ø²ÙŠØ±Ø©|Ø¨Ø§ÙƒØ³ØªØ§Ù†|Ø¥Ø³Ù„Ø§Ù… Ø¢Ø¨Ø§Ø¯|Ø§Ø³Ù„Ø§Ù… Ø¢Ø¨Ø§Ø¯')";
    ...
    $reclassify_row = function(array $row) use ($wpdb, $table) {
        ...
        if (!empty($manual_state['enabled'])) {
            $locked_actor = (string)($row['actor_v2'] ?? 'ÙØ§Ø¹Ù„ ØºÙŠØ± Ù…Ø­Ø³ÙˆÙ…');
            $locked_region = (string)($row['region'] ?? 'ØºÙŠØ± Ù…Ø­Ø¯Ø¯');
            $locked_intel = (string)($row['intel_type'] ?? 'Ø¹Ø§Ù…');
